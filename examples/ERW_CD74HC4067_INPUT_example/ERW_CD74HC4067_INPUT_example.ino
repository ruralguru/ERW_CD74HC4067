...
//Definitions 
#define muxInOut A0 // Define demux input or output pin. 
...
void setup() // Setup -------------------------------------------------------------
{
  ...
  pinMode(muxInOut,INPUT); //Tell if INPUT or OUTPUT
...
void loop() // Pete & repete ------------------------------------------------------
{
  if(millis()-Timer>=timeStep) // Returns true once timestep has passed.
  {
    ...
    unsigned int ADCvalue = analogRead(muxInOut); // Read analog Value.
    float voltage = ADCvalue; // need a float value.
    voltage *= 4.65; // Multiply by 4.65 if using usb power supply.
    //voltage *= 4.9; // Multiply by 4.9 if using external power supply.  
    voltage /= 1023.0;
    Serial.print(voltage); // Display voltage.
    Serial.print("V"); // Display Units.
    Serial.print("\t"); // Give space between values.
    if(chSelect==15)
    {
